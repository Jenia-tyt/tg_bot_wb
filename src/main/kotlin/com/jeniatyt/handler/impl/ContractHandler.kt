...
@Service
class ContractHandler(private val messageService: MessageService) : Handler {

    override fun accept(command: String, userId: Long?, activity: Activity?): Boolean {
        return MainCommand.CONTRACT.getCommand() == command
    }

    override fun handle(update: Update, userId: Long?, activity: Activity?) {
        val chatId = update.getChatIdAsString()
        messageService.sendMessageDocument(chatId, FilesName.CONTRACT)
        messageService.sendMessageWithKeyboard(chatId, Button.WITHOUT_CONTRACT_BUTTON, Message.PRICE_LOGISTICS)
    }
}
