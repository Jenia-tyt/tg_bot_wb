...
@Service
class ScheduleTravelHandler(private val messageService: MessageService) : Handler {

    override fun accept(command: String, userId: Long?, activity: Activity?): Boolean {
        return MainCommand.SCHEDULE_TRAVEL.getCommand() == command
    }

    override fun handle(update: Update, userId: Long?, activity: Activity?) {
        val chatId = update.getChatIdAsString()
        messageService.sendMessageDocument(chatId, FilesName.SCHEDULE_TRAVEL_OZON)
        messageService.sendMessageDocument(chatId, FilesName.SCHEDULE_TRAVEL_WB)
        messageService.sendMessageWithKeyboard(chatId, Button.WITHOUT_SCHEDULE_TRAVEL_BUTTON, Message.PRICE_LOGISTICS)
    }
}
