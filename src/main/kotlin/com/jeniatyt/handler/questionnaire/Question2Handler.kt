...
class Question2Handler(
    ...
    override fun accept(command: String, userId: Long?, activity: Activity?): Boolean {
        return QuestionnaireCommand.Q1.getCommand() == command ||
            activity?.command != null && activity.command == QuestionnaireCommand.Q1.getCommand()
    }
    ...
    override fun handle(update: Update, userId: Long?, activity: Activity?) {
        ...
        questionnaireRepository.findById(userId).ifPresentOrElse(
            { questionnaire -> questionnaire.data?.q1 = messageText },
            {
                val entity = com.jeniatyt.entity.Questionnaire(userId, QuestionnaireData().apply { q1 = messageText })
                questionnaireRepository.save(entity)
            }
        )

        if (activity != null) {
            activity.command = QuestionnaireCommand.Q2.getCommand()
            actionRepository.save(activity)
        } else {
            val newActivity = Activity(userId, QuestionnaireCommand.Q2.getCommand())
            actionRepository.save(newActivity)
        }

        messageService.sendMessageWithKeyboard(
            update.getChatIdAsString(),
            Button.baseStep(MainCommand.INDIVIDUAL_CALCULATION.getCommand(), QuestionnaireCommand.Q2.getCommand()),
