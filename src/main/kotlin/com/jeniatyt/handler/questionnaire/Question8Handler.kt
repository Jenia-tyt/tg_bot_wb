...
class Question8Handler(
    ...
    override fun accept(command: String, userId: Long?, activity: Activity?): Boolean {
        return QuestionnaireCommand.Q7.getCommand() == command ||
            activity?.command != null && activity.command == QuestionnaireCommand.Q7.getCommand()
    }
    ...
    override fun handle(update: Update, userId: Long?, activity: Activity?) {
        ...
        questionnaireRepository.findById(userId).ifPresentOrElse(
            { questionnaire -> questionnaire.data?.q7 = messageText },
            {
                val entity = com.jeniatyt.entity.Questionnaire(userId, QuestionnaireData().apply { q7 = messageText })
                questionnaireRepository.save(entity)
            }
        )

        if (activity != null) {
            activity.command = QuestionnaireCommand.Q8.getCommand()
            actionRepository.save(activity)
        } else {
            val newActivity = Activity(userId, QuestionnaireCommand.Q8.getCommand())
            actionRepository.save(newActivity)
        }

        messageService.sendMessageWithKeyboard(
            update.getChatIdAsString(),
            Button.baseStep(QuestionnaireCommand.Q6.getCommand(), QuestionnaireCommand.Q8.getCommand()),
